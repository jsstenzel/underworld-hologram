...
public:
  ...
  static CObjectList findObject(const SObjectDesc& desc, std::shared_ptr<CRoom> room = nullptr);
  static std::shared_ptr<CRoom> currentRoom();
  static void setRoom(std::shared_ptr<CRoom> room);
  static std::shared_ptr<CRoom> findRoom(std::string name);
...
private:
  ...
  static std::vector<std::shared_ptr<CRoom>> m_allRooms;
